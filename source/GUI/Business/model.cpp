...
void Model::toTreeWidget(QTreeWidget* tree)
{
  ...
  element = addRoot("Filename", model->filename, tree, true);
  std::vector<link_DA> links = model->model.GetListsLinks();

  for (std::size_t i = 0; i < links.size(); i++)
  {
    QTreeWidgetItem* elementLink = addRoot("Link", "", tree);
    addChild(elementLink, QObject::tr("Name"),
             QString::fromStdString(links.at(i).name));
    // Link pose
    poseElement = addChild(elementLink, QObject::tr("Pose"), "", false);
    splitVector(QString::fromStdString(links.at(i).pose), poseElement);
    // Link inertial properties
    element = addChild(elementLink, QObject::tr("Inertial"), "", false);

    // Mass
    addChild(element, QObject::tr("Mass [kg]"),
             QString::fromStdString(links.at(i).inertialValues.GetMass()));

    poseElement = addChild(element, QObject::tr("Pose [m]"), "", false);
    splitVector(QString::fromStdString(links.at(i).inertialValues.GetPose()),
                poseElement);
    element = addChild(element, QObject::tr("Inertia [kg.m^2]"), "", false);
    addChild(element, QObject::tr("ixx"),
             QString::fromStdString(links.at(i).inertialValues.GetIxx()));
    addChild(element, QObject::tr("ixy"),
             QString::fromStdString(links.at(i).inertialValues.GetIxy()));
    addChild(element, QObject::tr("ixz"),
             QString::fromStdString(links.at(i).inertialValues.GetIxz()));
    addChild(element, QObject::tr("iyy"),
             QString::fromStdString(links.at(i).inertialValues.GetIyy()));
    addChild(element, QObject::tr("iyz"),
             QString::fromStdString(links.at(i).inertialValues.GetIyz()));
    addChild(element, QObject::tr("izz"),
             QString::fromStdString(links.at(i).inertialValues.GetIzz()));
    // Collision properties
    element = addChild(elementLink, QObject::tr("Collision"), "", false);
    addChild(element, QObject::tr("Name"),
             QString::fromStdString(links.at(i).collision.name));

    poseElement = addChild(element, QObject::tr("Pose"), "", false);
    splitVector(QString::fromStdString(links.at(i).collision.pose),
                poseElement);
    element = addChild(element, QObject::tr("Geometry"), "", false);
    element = addChild(element, QObject::tr("URI"),
                       QString::fromStdString(
                           links.at(i).collision.geometry->GetValues().at(0)));
    QTreeWidgetItem* elementVisual =
        addChild(elementLink, QObject::tr("Visual"), "", false);
    addChild(elementVisual, QObject::tr("Name"),
             QString::fromStdString(links.at(i).visual.name));
    poseElement = addChild(elementVisual, QObject::tr("Pose"), "", false);
    splitVector(QString::fromStdString(links.at(i).visual.pose), poseElement);
    element = addChild(elementVisual, QObject::tr("Geometry"), "", false);
    addChild(
        element, QObject::tr("URI"),
        QString::fromStdString(links.at(i).visual.geometry->GetValues().at(0)));

    element = addChild(elementVisual, QObject::tr("Material"), "", false);
    addChild(element, QObject::tr("Ambient"),
             QString::fromStdString(links.at(i).visual.material.GetAmbient()));
    addChild(element, QObject::tr("Diffuse"),
             QString::fromStdString(links.at(i).visual.material.GetDiffuse()));
    addChild(element, QObject::tr("Emissive"),
             QString::fromStdString(links.at(i).visual.material.GetEmissive()));
    addChild(element, QObject::tr("Specular"),
             QString::fromStdString(links.at(i).visual.material.GetSpecular()));
  }
  ...
  for (uint i = 0; i < joints.size(); i++)
  {
    ...
    addChild(element, QObject::tr("Type"),
             QString::fromStdString(joints.at(i).type));
    addChild(element, QObject::tr("Parent"),
             QString::fromStdString(joints.at(i).parent));
    addChild(element, QObject::tr("Child"),
             QString::fromStdString(joints.at(i).child));

    poseElement = addChild(element, QObject::tr("Pose"), "", false);
    splitVector(QString::fromStdString(joints.at(i).pose), poseElement);

    QTreeWidgetItem* axisElement =
        addChild(element, QObject::tr("Axis"), "", false);
    if (joints.at(i).one != NULL)
    {
      addChild(axisElement, QObject::tr("XYZ"),
               QString::fromStdString(joints.at(i).one->xyz));
      addChild(axisElement, QObject::tr("Damping"),
               QString::fromStdString(joints.at(i).one->damping));
      addChild(axisElement, QObject::tr("Friction"),
               QString::fromStdString(joints.at(i).one->friction));
      addChild(axisElement, QObject::tr("Upper limit"),
               QString::fromStdString(joints.at(i).one->upper));
      addChild(axisElement, QObject::tr("Lower limit"),
               QString::fromStdString(joints.at(i).one->lower));
      addChild(axisElement, QObject::tr("Effort limit"),
               QString::fromStdString(joints.at(i).one->effort));
      addChild(axisElement, QObject::tr("Limit velocity"),
               QString::fromStdString(joints.at(i).one->velocity));
    }

    if (joints.at(i).two != NULL)
    {
      QTreeWidgetItem* axis2Element =
          addChild(element, QObject::tr("Axis2"), "");
      addChild(axis2Element, QObject::tr("XYZ"),
               QString::fromStdString(joints.at(i).two->xyz));
      addChild(axis2Element, QObject::tr("Damping"),
               QString::fromStdString(joints.at(i).two->damping));
      addChild(axis2Element, QObject::tr("Friction"),
               QString::fromStdString(joints.at(i).two->friction));
      addChild(axis2Element, QObject::tr("Upper limit"),
               QString::fromStdString(joints.at(i).two->upper));
      addChild(axis2Element, QObject::tr("Lower limit"),
               QString::fromStdString(joints.at(i).two->lower));
      addChild(axis2Element, QObject::tr("Effort limit"),
               QString::fromStdString(joints.at(i).two->effort));
      addChild(axis2Element, QObject::tr("Limit velocity"),
  ...
  {
    element = addRoot("Plugin", "", tree);
    addChild(element, "Name", plugins.at(i).getName());
    addChild(element, "Filename", plugins.at(i).getFilename());
    QMap<QString, QString> paramMap = plugins.at(i).getParameters();
    for (auto i = paramMap.cbegin(); i != paramMap.cend(); ++i)
    {
      addChild(element, i.key(), i.value());
    }
  }
  ...
  for (std::size_t i = 0; i < sensors.size(); i++)
  {
    ...
    addChild(element, QObject::tr("Link"), QString::number(sensors.at(i).link));
    QTreeWidgetItem* modelPose = addChild(element, QObject::tr("Pose"), "");
    splitVector(sensors.at(i).pose, modelPose);
    addChild(element, QObject::tr("Update_rate"), sensors.at(i).update_rate);
    ...
    if (sensors.at(i).name == "gps")
    {
      ...
      addChild(pos_hor, "bias stddev",
               sensors.at(i).pos_horizontal.bias_stddev);
      ...
      addChild(vel_hor, "bias stddev",
               sensors.at(i).vel_horizontal.bias_stddev);
...
void Model::splitvector(std::string data, QTreeWidgetItem* Element,
                        bool editable)
{
