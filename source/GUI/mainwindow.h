...
protected:
  ...
  /**
   * @brief checkGazeboIsRunning Checks if there exists a Gazebo process
   * already running before starting a new simulation.
   *
   * If gazebo is running, the user will be asked if they want to close
   * the processes before starting the simulation.
   */
  void checkGazeboIsRunning();

  /**
   * @brief getStepTimeUs Get the simulation step time specified in micro
   * seconds.
   * @param found If different from null, indicates if the step time was
   * found or not.
   * @return Simulation step time in micro-seconds.
   */
  uint64_t getStepTimeUs(bool* found = nullptr) const;
