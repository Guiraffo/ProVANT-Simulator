...
JointsDialog::JointsDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::JointsDialog)
{
    ui->setupUi(this);

    std::system("/home/arthur/catkin_ws/src/ProVANT-Simulator_Developer/source/GUI/Shell/getjoints > output.txt");
...
void JointsDialog::on_buttonBox_accepted()
{
    ...
    for(int i = 0; i< ui->treeWidget->topLevelItemCount();i++)
    {
        ...
        std::string command = "rosservice call /gazebo/set_model_configuration '{model_name: \"newmodel\", joint_names:['"+
                item->text(0).toStdString() +"'], joint_positions:[" + item->text(1).toStdString() + "]}'";
        //qDebug() << QString::fromStdString(command);
        std::system(command.c_str());
    }
}

void JointsDialog::on_treeWidget_itemDoubleClicked(QTreeWidgetItem *item, int column)
{
