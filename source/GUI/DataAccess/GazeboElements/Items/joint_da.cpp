...
bool joint_DA::Read(QDomNode* document)
{
  ...
  name = document->toElement().attribute("name").toStdString();
  type = document->toElement().attribute("type").toStdString();

  pose = document->firstChildElement("pose").text().toStdString();
  parent = document->firstChildElement("parent").text().toStdString();
  child = document->firstChildElement("child").text().toStdString();

  one->Read(*document);
  if (!document->firstChildElement("axis2").isNull())
    two->Read(*document);
  else
    two = NULL;
  return false;
