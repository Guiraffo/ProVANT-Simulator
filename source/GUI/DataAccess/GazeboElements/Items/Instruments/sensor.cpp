...
bool sensor::Read(QDomNode *document,bool* out2)
{
    ...
    *out2 = false;
    ...
    if(name=="sonar") ReadSonar(&element);
    if(name == "gps") ReadGps(&element);
    if(name=="imu") ReadImu(&element);
    if(name=="magnetometer") ReadMagnetometer(&element);
    *out2 = true;
    return false;
}
...
void sensor::Write(QXmlStreamWriter *xml)
{
    ...
    if(name == "sonar") WriteSonar(xml);
    if(name == "gps") WriteGps(xml);
    if(name == "imu") WriteImu(xml);
    if(name == "magnetometer") WriteMagnetometer(xml);
    xml->writeEndElement();
...
void sensor::print()
{
    if(name == "sonar") printSonar();
    if(name == "gps") printGps();
    if(name == "imu") printImu();
    if(name == "magnetometer") printMagnetometer();
}
