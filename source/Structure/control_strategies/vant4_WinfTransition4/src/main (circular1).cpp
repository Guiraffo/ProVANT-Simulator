...
class vant4_WinfTransition4 : public Icontroller
{
        ...
        private: Eigen::VectorXd TrajetoriaReferenciaCompleta(double Tempo)
	{
		...
		else if(Tempo > TMaxVel)
		{
			Tempo = Tempo - TMaxVel;
			double R = 480.2;
			double T = 153.5;
			x = R*cos(2*pi/T*Tempo - pi/2) + R;
			y = R*sin(2*pi/T*Tempo - pi/2) + R;
			z = 2.0;
			phi = -0.1;
			theta = -0.03;
			psi = 2*pi*Tempo/T;
			xp = (2*R*pi*sin(pi/2 - (2*pi*Tempo)/T))/T;
			yp = (2*R*pi*cos(pi/2 - (2*pi*Tempo)/T))/T;
			zp = 0;
			psip = 2*pi/T;
