...
void temp_press::Load(sensors::SensorPtr _sensor, sdf::ElementPtr _sdf)
{
	  ...
	  TempStandardDeviation = XMLRead::ReadXMLDouble("TempStandardDeviation",_sdf); // get value of Temperature's standard deviation 
	  BaroOffset = XMLRead::ReadXMLDouble("BaroOffset",_sdf); // get value of Barometer's offset 
	  BaroStandardDeviation = XMLRead::ReadXMLDouble("BaroStandardDeviation",_sdf); // get value of Barometer's Standard deviation 
